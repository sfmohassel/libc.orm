...
namespace libc.orm.postgres.DdlProcessing.Postgres {
    public class PostgresProcessor : GenericProcessorBase {
        ...
        public override bool SchemaExists(string schemaName) {
            return Exists("select * from information_schema.schemata where schema_name = '{0}'",
                FormatToSafeSchemaName(schemaName));
        }
        public override bool TableExists(string schemaName, string tableName) {
            return Exists("select * from information_schema.tables where table_schema = '{0}' and table_name = '{1}'",
        ...
        public override bool ColumnExists(string schemaName, string tableName, string columnName) {
            return Exists(
                ...
                FormatToSafeSchemaName(schemaName), FormatToSafeName(tableName), FormatToSafeName(columnName));
        }
        public override bool ConstraintExists(string schemaName, string tableName, string constraintName) {
            return Exists(
                ...
                FormatToSafeSchemaName(schemaName), FormatToSafeName(tableName), FormatToSafeName(constraintName));
        }
        public override bool IndexExists(string schemaName, string tableName, string indexName) {
            return Exists(
                "select * from pg_catalog.pg_indexes where schemaname='{0}' and tablename = '{1}' and indexname = '{2}'",
                FormatToSafeSchemaName(schemaName), FormatToSafeName(tableName), FormatToSafeName(indexName));
        }
        public override bool SequenceExists(string schemaName, string sequenceName) {
            return Exists(
                ...
                FormatToSafeSchemaName(schemaName), FormatToSafeName(sequenceName));
        }
        ...
        private string FormatToSafeSchemaName(string schemaName) {
            ...
            if (!_quoter.Options.ForceQuote) schemaNameCased = schemaName?.ToLowerInvariant();
        ...
        private string FormatToSafeName(string sqlName) {
            ...
            if (!_quoter.Options.ForceQuote) sqlNameCased = sqlName?.ToLowerInvariant();
