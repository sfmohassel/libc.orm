...
namespace libc.orm.DatabaseMigration.Abstractions.Model
{
    ...
    public static class CloneExtensions
    {
        ...
        public static void CloneTo(this IEnumerable<KeyValuePair<string, object>> dict,
            IDictionary<string, object> target)
        {
            var clonedItems = dict
                .Select(x => Tuple.Create(x.Key, (x.Value as ICloneable)?.Clone() ?? x.Value));

            foreach (var clonedItem in clonedItems) target[clonedItem.Item1] = clonedItem.Item2;
        }
    }
}