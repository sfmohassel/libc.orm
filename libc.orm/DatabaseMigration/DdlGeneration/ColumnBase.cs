...
namespace libc.orm.DatabaseMigration.DdlGeneration
{
    ...
    public abstract class ColumnBase : IColumn
    {
        ...
        {
            return string.Format("FK_{0}_{1}", foreignKey.PrimaryTable.Substring(0, 5),
                foreignKey.ForeignTable.Substring(0, 5));
        }
            ...
            Func<ForeignKeyDefinition, string> fkNameGeneration)
        {
            ...
            foreach (var column in foreignKey.PrimaryColumns) primaryColumns.Add(Quoter.QuoteColumnName(column));
            foreach (var column in foreignKey.ForeignColumns) foreignColumns.Add(Quoter.QuoteColumnName(column));
        ...
        {
            if (!column.Type.HasValue) return column.CustomType;
        ...
        {
            if (column.IsNullable == true) return string.Empty;
        ...
        {
            if (!string.IsNullOrEmpty(column.CollationName)) return "COLLATE " + column.CollationName;
        ...
        public virtual string AddPrimaryKeyConstraint(string tableName, IEnumerable<ColumnDefinition> primaryKeyColumns)
        {
            ...
            return string.Format(", {0}PRIMARY KEY ({1})", GetPrimaryKeyConstraintName(pkColDefs, tableName),
                keyColumns);
        }
        ...
        protected virtual string GetPrimaryKeyConstraintName(IEnumerable<ColumnDefinition> primaryKeyColumns,
            string tableName)
        {
            var primaryKeyName = primaryKeyColumns.Select(x => x.PrimaryKeyName).FirstOrDefault();

            if (string.IsNullOrEmpty(primaryKeyName)) return string.Empty;
