...
namespace libc.orm.sqlserver.DdlProcessing {
    public class SqlServer2012ProcessorFactory : IProcessorFactory {
        public IProcessor Create(ProcessorOptions options, ILogger logger) {
            return new SqlServer2012Processor(logger,
                new SqlServer2012Generator(new SqlServer2008Quoter(), new GeneratorOptions()), options);
        }
    }
}