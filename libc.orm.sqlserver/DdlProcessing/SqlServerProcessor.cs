...
namespace libc.orm.sqlserver.DdlProcessing
{
    public class SqlServerProcessor : GenericProcessorBase
    {
        ...
        private const string TABLE_EXISTS =
            "SELECT 1 WHERE EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}')";

        private const string COLUMN_EXISTS =
        ...
        private const string CONSTRAINT_EXISTS =
            "SELECT 1 WHERE EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_CATALOG = DB_NAME() AND TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}' AND CONSTRAINT_NAME = '{2}')";

        private const string INDEX_EXISTS =
            "SELECT 1 WHERE EXISTS (SELECT * FROM sys.indexes WHERE name = '{0}' and object_id=OBJECT_ID('{1}.{2}'))";
        ...
        public override bool IndexExists(string schemaName, string tableName, string indexName)
        {
            return Exists(INDEX_EXISTS,
                FormatHelper.FormatSqlEscape(indexName), SafeSchemaName(schemaName),
                FormatHelper.FormatSqlEscape(tableName));
        }
